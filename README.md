...
ブラウザで `http://127.0.0.1:8000/` を直接開くだけでは起動しません。先に `open-sedori-alert.cmd` を開いてください。

このファイルを開くと、サーバー用PowerShellが起動し、準備ができたらブラウザで `http://127.0.0.1:8000/` を開きます。サーバー用PowerShellを閉じるとサイトも止まります。

起動中の画面には、成功または失敗理由が表示されます。サイトを使っている間は、その画面を閉じないでください。終了するときだけ、その画面でEnterキーを押します。
...
Discord通知は、商品名、URL、ショップ、キーワード、価格、カテゴリ、商品画像をembed形式で表示します。
